...
def guardar_informe(tabla: pd.DataFrame):
    ...
    for direccion in ["alza", "baja"]:
        ...
        lineas.append("")
        lineas.append("| Rango de verde | Nº cruces | % a favor | Retorno medio a favor |")
        lineas.append("|---|---|---|---|")
        for _, row in tabla[tabla["direccion"] == direccion].iterrows():
            n = row["n_cruces"]
            pct = f"{row['pct_a_favor']}%" if row["pct_a_favor"] is not None else "-"
            ret = f"{row['retorno_medio_pct']}%" if row["retorno_medio_pct"] is not None else "-"
            lineas.append(f"| {row['rango_verde']} | {n} | {pct} | {ret} |")
        lineas.append("")

    lineas.append(
        "_Si el rango 0-50 (alza) / -50-0 (baja) es realmente el mejor, esas filas deberian "
        "tener el % a favor y el retorno medio mas altos dentro de su direccion. Si no es asi, "
        "el umbral deberia ajustarse al rango que de verdad rinda mejor en esta tabla._"
    )
